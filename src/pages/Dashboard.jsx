...
export default function Dashboard() {
  ...
  const [branchStats, setBranchStats] = useState([]); // per-branch cards
  const [branchBarData, setBranchBarData] = useState([]); // comparison chart
  ...
  // Reload pie charts when branch filter changes
  useEffect(() => {
    if (!loading) loadPies();
  }, [selectedBranch]);
  ...
  const loadDashboard = async () => {
    try {
      ...
      // Per-branch stats for cards + bar chart (last 7 days)
      const branchResults = await Promise.all(
        branchList.map(async (b) => {
          const res = await API.get(
            `/reports/sales?from=${weekFrom}&to=${now}&branchId=${b.id}`
          );
      ...
      // Load pies (respects selectedBranch but on first load it's "all")
      await loadPies();
    } catch (err) {
  ...
  return (
    <div className="space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {branchStats.map((b, i) => (
              <div
                key={b.id}
                onClick={() => setSelectedBranch(String(b.id) === String(selectedBranch) ? "all" : String(b.id))}
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {b.location && (
                    <span className="text-xs text-slate-400 ml-auto truncate">📍 {b.location}</span>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    <p className="text-slate-400">Revenue</p>
                    <p className="font-bold text-green-600">₹ {Number(b.revenue).toLocaleString("en-IN")}</p>
                  </div>
                  <div>
                    <p className="text-slate-400">Invoices</p>
                    <p className="font-bold text-blue-600">{b.invoices}</p>
                  </div>
                  <div>
                    <p className="text-slate-400">Customers</p>
                    <p className="font-bold text-amber-600">{b.customers}</p>
                  </div>
                  <div>
                    <p className="text-slate-400">Items Sold</p>
                    <p className="font-bold text-purple-600">{b.productsSold}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* ── Pie Charts (filtered by selected branch) ── */}
      <div>
        {selectedBranch !== "all" && (
          <p className="text-sm text-blue-600 font-semibold mb-3">
            ...
            <button
              ...
              className="ml-3 text-slate-400 hover:text-slate-600 underline font-normal"
            >
              Clear filter
            </button>
          </p>
