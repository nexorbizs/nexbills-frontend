...
export default function Products() {
  ...
  const formFields = [
    ...
    { key: "barcode",  placeholder: "BARCODE (EAN/UPC)" }, // ⭐ NEW
        ...
        <table className="w-full min-w-[900px] text-sm">
          <thead className="bg-slate-100">
            <tr>
              ...
              <th className="p-3 text-left">Barcode</th> {/* ⭐ NEW */}
          ...
          <tbody>
            {products.map((p, index) => (
              <tr key={p.id} className={`border-t ${index === activeIndex ? "bg-blue-100" : "hover:bg-slate-50"}`}>
                <td className="p-3 font-medium">{p.name}</td>
                <td className="p-3 text-slate-500">{p.sku}</td>
                <td className="p-3 text-slate-500">{p.hsn}</td>
                <td className="p-3 text-slate-500 font-mono text-xs">{p.barcode || "—"}</td> {/* ⭐ NEW */}
